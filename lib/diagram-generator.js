...
- Merge duplicate or low-value items. Keep the diagram focused and teachable.
...
auto, flowchart, mindmap, orgchart, sequence, class, er, gantt, timeline, tree, network, architecture, dataflow, state, swimlane, concept, fishbone, swot, pyramid, funnel, venn, matrix, infographic
...
export async function generateDiagramElements(config, userInput, chartType = 'auto') {
  ...
  if (parsedPlan.ok && Array.isArray(parsedPlan.value)) {
    return {
      ...
      diagramType: chartType !== 'auto' ? chartType : 'flowchart',
      mode: 'legacy-array',
    };
  }

  if (parsedPlan.ok && parsedPlan.value && typeof parsedPlan.value === 'object') {
    const plan = normalizeDiagramPlan(parsedPlan.value, chartType);
...
function buildPlanUserPrompt(userInput, chartType) {
  ...
  const chartInstruction = chartType && chartType !== 'auto'
  ...
  return [
    ...
    'Keep the structure compact, readable, and useful after deterministic rendering.',
    `User input:\n${textInput}`,
  ].join('\n\n');
}
...
export function normalizeDiagramPlan(rawPlan, requestedType = 'auto') {
  const sections = normalizeSections(rawPlan.sections);
  const normalizedType = requestedType !== 'auto'
    ? sanitizeDiagramType(requestedType, 'flowchart')
    : sanitizeDiagramType(rawPlan.diagramType, inferDiagramType(rawPlan, sections));
  ...
  const enrichedSections = attachSectionItems(sections, nodesResult.nodes);
...
function renderSectionedDiagram(plan) {
  ...
  const nodeElements = plan.nodes.map((node) => {
    const position = nodePositions.get(node.id) || { x: 120, y: 140 };
    return createNodeElement(node, position.x, position.y, plan.diagramType, measuredNodes.get(node.id));
  });

  const arrows = buildEdgeElements(plan, nodeElements);
...
function renderStructuredDiagram(plan) {
  ...
  nodes.forEach((node) => {
    ...
    const element = createNodeElement(node, position.x, position.y, plan.diagramType);
    nodeElements.push(element);

    if (node.sectionId && sectionChildren.has(node.sectionId)) {
      sectionChildren.get(node.sectionId).push(element.id);
    }
  });
...
function createNodeElement(node, x, y, diagramType, measured = null) {
  ...
  const palette = colorPaletteForNode(node.kind);
...
function buildNodeLabel(node) {
  const detailLines = node.details.slice(0, 3).map((detail) => `• ${detail}`);
...
export function finalizeElementArray(elements, diagramType, options = {}) {
  ...
  const optimized = optimizeExcalidrawCode(JSON.stringify(elements), {
    chartType: diagramType,
    skipLayout: Boolean(options.preserveLayout),
  });
  const parsed = safeParseJsonWithRepair(optimized);
  return parsed.ok && Array.isArray(parsed.value) ? parsed.value : elements;
}
...
function normalizeDetails(value) {
  if (Array.isArray(value)) {
    return value.map((item) => cleanLabel(item)).filter(Boolean).slice(0, 4);
  }
