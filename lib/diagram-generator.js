...
function renderPyramidDiagram(plan) {
  ...
  items.forEach((item, index) => {
    ...
    elements.push({
      ...
      y,
      width,
      height: stepHeight - 12,
      strokeColor: '#1d4ed8',
      backgroundColor: ['#dbeafe', '#bfdbfe', '#93c5fd', '#60a5fa'][index % 4],
      label: { text: item.label, fontSize: 16 },
    });
  });
...
function getShapeDiagramItems(plan) {
  if (plan.sections.length > 0) {
    return plan.sections.map((section) => ({ id: section.id, label: section.title }));
  }

  return plan.nodes.slice(0, 5).map((node) => ({ id: node.id, label: node.label }));
}
