 ...
 */
export function optimizeExcalidrawCode(codeString, chartTypeOrOptions = 'flowchart', maybeOptions = {}) {
  if (!codeString || typeof codeString !== 'string') {
    return codeString;
  }

  try {
    const chartType = typeof chartTypeOrOptions === 'string'
      ? chartTypeOrOptions
      : chartTypeOrOptions?.chartType || 'flowchart';
    const options = typeof chartTypeOrOptions === 'string'
      ? maybeOptions
      : chartTypeOrOptions || {};
    ...
    const preparedElements = options.skipLayout
      ? normalizedElements
      : applyLayoutEngine(normalizedElements, chartType);
